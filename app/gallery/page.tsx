      ...
      <div className="bg-black/80 border border-white/10 rounded-3xl overflow-hidden backdrop-blur-md transition-colors hover:border-white/30 h-full flex flex-col justify-between">
        ...
        <div className="p-6">
            ...
            <div className="flex gap-3 items-center">
              <Avatar className="h-10 w-10 border-2 border-emerald-500/30">
                <AvatarImage
                  src={
                    post.author.avatar ||
                    `https://api.dicebear.com/7.x/personas/svg?seed=${post.author.name}`
                  }
                />
              ...
              <div>
                ...
                <div className="text-[10px] text-white/40">
                  {Math.floor(
                    (Date.now() - post.timestamp.getTime()) / 3600000
                  )}
                  h ago
                </div>
              </div>
          ...
          <div className="flex items-center gap-2 mb-3">
            {post.format_type === 'Comic Book' ? (
              <Badge className="bg-blue-500/10 text-blue-400 border-none flex items-center gap-1">
                <Layers className="w-3 h-3" /> Comic
              </Badge>
            ) : (
              <Badge className="bg-amber-500/10 text-amber-400 border-none flex items-center gap-1">
                <Book className="w-3 h-3" /> Storybook
              </Badge>
            )}
            {post.genre?.map((g) => (
              <Badge
        ...
        <div className="flex items-center justify-between text-white/50 px-6 pb-6 pt-2">
          <div className="flex items-center gap-1 bg-white/5 rounded-full p-1 border border-white/10">
            <button
              onClick={() =>
                onVote(post.id, post.userVote === 'up' ? null : 'up')
              }
          ...
          {post.file_url ? (
            <Button
              variant="ghost"
              size="sm"
              className="rounded-full hover:bg-emerald-500/20 hover:text-emerald-400"
...
export default function GalleryPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'Storybook' | 'Comic Book'>(
    'all'
  );
  ...
  useEffect(() => {
    fetch(
      `${process.env.NEXT_PUBLIC_API_URL || 'https://groqtales-backend-api.onrender.com'}/api/v1/stories?limit=40`
    )
      .then((r) => {
      ...
      .then((json) => {
        ...
        const galleryItems = data.filter(
          (story: any) => story.is_minted === false || !story.is_minted
        );
  ...
  const filtered = posts.filter(
    (p) => filter === 'all' || p.format_type === filter
  );
      ...
      <div className="container mx-auto px-6 py-12 relative z-10 max-w-7xl">
        <div className="mb-12 text-center md:text-left">
          ...
          <p className="text-white/50 text-lg max-w-2xl">
            Explore unminted masterpieces uploaded by the community. Validated
            and synthesized by our Synoptic AI framework.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-24 bg-white/5 rounded-3xl border border-white/10 flex flex-col items-center">
            ...
            <p className="text-white/50">
              No verified pieces found matching your criteria. Be the first to
              upload.
            </p>
          </div>
        ) : (
          <div className="columns-1 md:columns-2 lg:columns-3 gap-6">
            <AnimatePresence>
              {filtered.map((p) => (
                <GalleryCard key={p.id} post={p} onVote={handleVote} />
              ))}
            </AnimatePresence>
          </div>
        )}
