...
export default function AccessControlPage() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch(
        'https://groqtales-backend-api.onrender.com/api/v1/admin/access/users',
        {
          headers: {
  ...
  const confirmRoleChange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        'https://groqtales-backend-api.onrender.com/api/v1/admin/access/roles',
        {
          method: 'PATCH',
          headers: {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-white/60 mt-1">
            Manage platform users, view connections, and modify administrative
            roles.
          </p>
        </div>
      ...
      <Card className="bg-black/40 border-white/10 text-white shadow-xl backdrop-blur-sm">
        <CardHeader>
          ...
          <CardDescription className="text-white/50">
            {filteredUsers.length} user(s) found. Role changes require master
            ...
            <Table>
              <TableHeader className="bg-white/5 disabled:hover:bg-transparent">
                <TableRow className="border-white/10 hover:bg-transparent">
                  ...
                  <TableHead className="text-white/60 font-medium hidden md:table-cell">
                    Identity
                  </TableHead>
                  <TableHead className="text-white/60 font-medium">
                    Stories
                    ...
                    >
                      <TableCell className="font-medium">
                        <div className="flex flex-col">
                          ...
                          <span
                            className="text-xs text-white/40 font-mono truncate max-w-[120px]"
                            title={user._id}
                          >
                            {user._id}
                          </span>
                      ...
                      <TableCell className="text-right">
                        <Select
                          value={user.role}
                          onValueChange={(val) =>
                            initiateRoleChange(user._id, val)
                          }
                        >
        ...
        <DialogContent className="bg-zinc-950 border-white/10 text-white sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-xl">
              <ShieldAlert className="h-5 w-5 text-red-500" /> Security
              Verification
            </DialogTitle>
            <DialogDescription className="text-white/60">
              You are altering a user's administrative access. Please enter the
              master admin password to confirm this action.
            </DialogDescription>
          </DialogHeader>
