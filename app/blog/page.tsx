...
const blogs = [
  {
    ...
    excerpt:
      'Most NFT projects treated lore as marketing copy. What if the story was the first-class asset? And what if AI + on-chain rails made it fast to create, fair to share, and transparent to curate?',
    author: 'Mantej Singh',
    date: 'March 2, 2026',
    readTime: '6 min',
    ...
    <div className="min-h-screen bg-background dark:dark-premium-bg selection:bg-primary/30 pb-24">
      <div className="container max-w-6xl mx-auto px-4 py-16 md:py-24">
        <header className="mb-16 text-center">
          <h1
            className="text-4xl md:text-6xl font-extrabold tracking-tight mb-6 text-transparent bg-clip-text bg-gradient-to-br from-white via-white to-white/60 font-display"
            style={{ fontFamily: 'var(--font-comic)' }}
          >
            The Developer Blog
          </h1>
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Deep dives into the engineering, vision, and ecosystem behind
            GroqTales.
          </p>
        </header>
              ...
              <Link href={`/blog/${blog.slug}`} className="block h-full group">
                <div className="flex flex-col h-full bg-[#0a0e1a]/80 border border-white/10 rounded-2xl overflow-hidden hover:border-primary/50 transition-all duration-300 shadow-lg hover:shadow-[0_8px_30px_rgba(255,77,109,0.15)] transform-gpu hover:-translate-y-1">
                  ...
                  <div className="relative h-48 w-full bg-[#0f172a] overflow-hidden flex items-center justify-center">
                    ...
                    <div className="absolute bottom-4 left-4 z-20 flex gap-2">
                      {blog.tags.map((tag) => (
                        <span
                          key={tag}
                          className="px-2.5 py-1 rounded bg-black/60 backdrop-blur-md border border-white/10 text-[10px] font-bold text-white/90 uppercase tracking-widest"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    ...
                    <h2
                      className="text-xl md:text-2xl font-bold text-white group-hover:text-primary transition-colors mb-3 line-clamp-2 leading-tight font-display"
                      style={{ fontFamily: 'var(--font-comic)' }}
                    >
                      {blog.title}
                    </h2>
