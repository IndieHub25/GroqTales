...
export default function StoryPageClient({
      ...
      try {
        ...
        if (!storyData) {
          ...
          toast({
            ...
            description: "The story doesn't exist.",
            variant: 'destructive',
          });
          return;
        }
  ...
  const handleCommentSubmit = (e: React.FormEvent) => {
    ...
    if (!account) {
      toast({
        title: 'Wallet Required',
        description: 'Connect wallet to comment.',
      });
      return;
    }
  ...
  const handleCommentLike = (commentId: string) => {
    if (!account) {
      toast({
        title: 'Wallet Required',
        description: 'Connect wallet to like.',
      });
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0c] text-[#e0e0e0] font-serif transition-colors duration-500">
        ...
        <div className="max-w-3xl mx-auto py-24 px-6 md:px-12">
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-[#f0f0f0] tracking-tight">
            {story.title}
          </h1>
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-3">
              <Button
                ...
                className="h-14 px-8 rounded-full bg-white text-black hover:bg-white/90 font-semibold text-lg shadow-[0_0_20px_rgba(255,255,255,0.2)]"
              >
              ...
              <Button
                ...
                disabled={isSharing}
                variant="outline"
                className="h-14 w-14 rounded-full border-white/20 text-white hover:bg-white/10 p-0 flex items-center justify-center"
              >
              ...
              <div className="h-14 flex items-center gap-4 px-6 rounded-full bg-white/5 border border-white/10 text-white/70">
                <button
                  ...
                  className={`flex items-center gap-2 hover:text-emerald-400 transition-colors ${voteStatus === 'upvote' ? 'text-emerald-400' : ''}`}
                >
      ...
      <section className="relative z-10 container mx-auto px-6 py-12">
        ...
        >
          <TabsList className="flex w-full bg-white/5 p-1 rounded-full border border-white/10 mb-8 h-14">
            <TabsTrigger
              value="story"
              className="flex-1 rounded-full data-[state=active]:bg-white data-[state=active]:text-black text-white/60 font-semibold text-base transition-all"
            >
              The Lore
            </TabsTrigger>
            <TabsTrigger
                ...
                <div className="bg-white/5 border border-white/10 rounded-3xl p-8 md:p-12 backdrop-blur-sm">
                  ...
                  <div className="mt-10 pt-10 border-t border-white/10 flex justify-center">
                    <p className="text-white/40 italic">
                      Enter Reading Mode to experience the full continuity.
                    </p>
                  </div>
                </div>
              </TabsContent>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-sm space-y-6">
                    <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                      <Cpu className="text-emerald-400" /> Ownership
                    </h3>
                    ...
                    <div className="flex justify-between items-center py-3 border-b border-white/5">
                      ...
                      <span className="font-medium flex items-center gap-1.5">
                        <Hexagon className="w-4 h-4 text-purple-400" /> Monad
                        Testnet
                      </span>
                    </div>
                    <div className="flex justify-between items-center py-3 border-b border-white/5">
                      <span className="text-white/50">Creator Royalty</span>
                      <span className="font-medium">10%</span>
                    </div>
                    <Button className="w-full mt-4 h-12 bg-emerald-500 hover:bg-emerald-400 text-black font-bold text-base rounded-xl">
                      Purchase Asset
                    </Button>
                  </div>
                  <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-sm space-y-6">
                    <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                      <Sparkles className="text-blue-400" /> Generation Info
                    </h3>
                    <div className="flex justify-between items-center py-3 border-b border-white/5">
                      <span className="text-white/50">Engine</span>
                      <span className="font-medium">Groq LLM</span>
                    </div>
                    <div className="flex justify-between items-center py-3 border-b border-white/5">
                      <span className="text-white/50">Mint Date</span>
                      <span className="font-medium">
                        {story?.mintDate
                          ? new Date(story.mintDate).toLocaleDateString()
                          : '—'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center py-3 border-b border-white/5">
                      <span className="text-white/50">Content Rating</span>
                      <span className="font-medium bg-white/10 px-2 py-1 rounded text-xs">
                        Safe
                      </span>
                    </div>
                    ...
                    <div className="mt-8 pt-8 border-t border-white/10">
                      <Button
                        variant="outline"
                        className="w-full h-12 border-white/20 hover:bg-white/10 text-white rounded-xl"
                        onClick={() => {
                          window.location.href = `/create/ai-story?source=story&genre=${encodeURIComponent(story.genre || 'fantasy')}&format=nft`;
                        }}
                      >
                        <PenSquare className="h-4 w-4 mr-2" /> Remix / Fork
                        Story
                      </Button>
                    </div>
                  </div>
              ...
              <TabsContent value="community" className="mt-0 outline-none">
                <div className="bg-white/5 border border-white/10 rounded-3xl p-8 md:p-12 backdrop-blur-sm">
                  ...
                  <p className="text-white/50 mb-8">
                    Join the discussion with other collectors and readers.
                  </p>
                  ...
                  >
                    ...
                    />
                    <div className="absolute bottom-4 right-4 text-xs text-white/30 hidden md:block">
                      Press bold/italic tools to format
                    </div>
                    <div className="flex justify-end mt-4">
                      <Button
                        type="submit"
                        disabled={!commentText.trim() || !account}
                        className="bg-white text-black hover:bg-white/90 rounded-full px-8 h-12 font-semibold"
                      >
                        Post Insight
                      </Button>
                    </div>
                          ...
                          <div className="flex gap-4">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                {comment.isVerified && (
                                  <VerifiedIcon className="h-3.5 w-3.5 text-emerald-400" />
                                )}
                              ...
                              <Button
                                ...
                                disabled={!account}
                                className="h-8 px-2 text-white/50 hover:text-emerald-400 hover:bg-white/5 -ml-2 rounded-lg"
                              >
                                <Heart
                                  className={`h-3.5 w-3.5 mr-1.5 ${likedComments.has(comment.id) ? 'fill-current text-emerald-400' : ''}`}
                                />{' '}
                                {comment.likes}
                              </Button>
                            </div>
