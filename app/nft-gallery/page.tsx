  ...
  ({
    ...
    return (
      ...
      >
        ...
        <div
          className={`relative bg-white/5 border border-white/10 backdrop-blur-md rounded-3xl overflow-hidden transition-all duration-500 group-hover:-translate-y-2 group-hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.5)] ${isLegendary ? 'group-hover:border-amber-500/50' : 'group-hover:border-white/30'}`}
        >
          <div className="relative h-[320px] w-full overflow-hidden">
            ...
            <div className="absolute top-4 left-4 flex gap-2">
              {nft.isTop10 && (
                <Badge className="bg-amber-500/20 text-amber-400 border border-amber-500/50 backdrop-blur-sm shadow-none">
            ...
            <Badge
              variant="outline"
              className={`absolute top-4 right-4 backdrop-blur-sm border border-white/20 ${isLegendary ? 'text-amber-400 bg-amber-400/10' : 'text-white/80 bg-white/10'}`}
            >
              {nft.rarity || 'Common'}
            </Badge>
          ...
          <div className="p-5">
            <div className="flex justify-between items-center mb-4">
              <Badge className="bg-white/5 text-white/50 border-white/10 hover:bg-white/10">
                {nft.genre}
              </Badge>
              <div className="font-bold text-lg text-emerald-400 flex items-center gap-1.5">
                <Hexagon className="w-4 h-4" /> {nft.price}
              </div>
            </div>
            <div className="flex gap-2">
              <Button
                ...
                className="flex-1 rounded-xl bg-white/5 border-white/10 hover:bg-white/10 hover:text-white text-white/70"
              >
                <Heart className="w-4 h-4 mr-2" /> {nft.likes}
              </Button>
              <Button
                size="sm"
                className="flex-1 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-black font-semibold shadow-[0_0_15px_rgba(16,185,129,0.2)]"
              >
                Purchase
              </Button>
            </div>
...
export default function NFTGalleryPage() {
  ...
  const handleLike = (id: string) => {
    if (!connected) {
      toast({
        title: 'Auth Required',
        description: 'Connect wallet to verify action.',
      });
      return;
    }
    setNfts((prev) =>
    ...
    toast({
      ...
      className: 'bg-black border-white/10 text-emerald-400',
    });
  };
      ...
      <div className="container mx-auto px-6 py-12 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-12">
          <div>
            ...
            <p className="text-lg text-white/50 max-w-xl">
              Acquire legendary artifacts and narrative blueprints forged by the
              neural engine.
            </p>
          </div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-4 md:p-6 backdrop-blur-md mb-12 flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <Input
              placeholder="Search artifacts or creators..."
              value={searchTerm}
        ...
        {filteredNFTs.length === 0 && (
          <div className="text-center py-24 bg-white/5 border border-white/10 rounded-3xl mt-6">
            ...
            <p className="text-white/40">
              Adjust your sensory filters to locate artifacts.
            </p>
          </div>
        )}
      </div>
              ...
              <div className="p-8 md:p-10 flex flex-col h-full overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
                    ...
                    <div className="text-2xl font-bold text-emerald-400 flex items-center gap-1">
                      <Hexagon className="w-5 h-5" /> {selectedNFT.price}
                    </div>
                  </div>
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
                    <div className="text-white/40 text-xs uppercase tracking-widest mb-1">
                ...
                <div className="flex gap-4 mt-auto">
                  <Button
                    ...
                    variant="outline"
                    className="flex-1 h-14 rounded-full border-white/20 bg-white/5 hover:bg-white/10"
                  >
