...
admin.use('*', async (c, next) => {
  ...
  try {
    ...
    return c.json(
      { error: 'Internal Server Error verifying admin status' },
      500
    );
  }
});
...
admin.put('/approve/:id', async (c) => {
  ...
  try {
    const result = await db
      .prepare('UPDATE stories SET review_status = ? WHERE id = ?')
      .bind('verified', id)
      .run();
    ...
    if (result.meta?.changes === 0) {
      return c.json(
        { error: 'Not Found: Story does not exist or is already verified' },
        404
      );
    }
    ...
    return c.json({
      success: true,
      message: 'Story officially verified and live on marketplace',
    });
  } catch (error) {
