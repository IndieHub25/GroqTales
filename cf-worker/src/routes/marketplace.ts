...
marketplace.post('/list', async (c) => {
  ...
  try {
    ...
    const listCurrency =
      currency && allowedCurrencies.includes(currency.toUpperCase())
    ...
    return c.json(
      { success: true, message: 'Listing created', listingId },
      201
    );
  } catch (error) {
    return c.json(
      { error: 'Failed to create listing or invalid payload format' },
      500
    );
  }
});
