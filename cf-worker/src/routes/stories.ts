...
stories.post('/', async (c) => {
  ...
  try {
    ...
    return c.json(
      { success: true, message: 'Story created', id: storyId },
      201
    );
  } catch (error) {
    return c.json(
      { error: 'Failed to create story or invalid JSON payload' },
      500
    );
  }
});
...
stories.get('/:id', async (c) => {
  ...
  try {
    ...
    const story = await db
      .prepare('SELECT * FROM stories WHERE id = ? AND review_status = ?')
      .bind(id, 'verified')
      .first();
