...
describe('Bug Exploration: Array formatType in Stories Route', () => {
  ...
  test('should show expected behavior after fix - normalize to string', () => {
    ...
    const formatTypeCapitalized =
      normalizedFormatType.charAt(0).toUpperCase() +
      normalizedFormatType.slice(1);
  ...
  test('should verify groqService.generate is called with correct parameter types', async () => {
    ...
    const normalizedParams = {
      prompt: Array.isArray(requestBody.prompt)
        ? requestBody.prompt[0]
        : requestBody.prompt,
      genre: Array.isArray(requestBody.genre)
        ? requestBody.genre[0]
        : requestBody.genre,
      formatType: Array.isArray(requestBody.formatType)
        ? requestBody.formatType[0]
        : requestBody.formatType,
    };
