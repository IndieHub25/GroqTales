...
describe('Groq Service Module', () => {
  ...
  describe('Model Configuration', () => {
    ...
    test('exports token budgets for all content lengths', () => {
      expect(groqService.TOKEN_BUDGETS.short).toBeLessThan(
        groqService.TOKEN_BUDGETS.medium
      );
      expect(groqService.TOKEN_BUDGETS.medium).toBeLessThan(
        groqService.TOKEN_BUDGETS.long
      );
  ...
  describe('Prompt Builders', () => {
    ...
    test('buildSynopsisPrompt is concise (token-efficient)', () => {
      const prompt = groqService.buildSynopsisPrompt(
        'story content',
        'fantasy',
        'novel'
      );
    ...
    test('buildImprovementPrompt truncates long content', () => {
      ...
      const prompt = groqService.buildImprovementPrompt(
        longContent,
        'dialogue'
      );
  ...
  describe('generate()', () => {
    ...
    test('uses comic system prompt for comic formatType', async () => {
      ...
      expect(callBody.messages[0].content).toContain(
        groqService.SYSTEM_PROMPTS.comic
      );
    });
  });
  ...
  describe('analyze()', () => {
    test('returns parsed JSON analysis', async () => {
      ...
      const result = await groqService.analyze({
    ...
    test('handles non-JSON analysis response gracefully', async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        json: async () => ({
          choices: [
            { message: { content: 'This is a great story about adventure.' } },
          ],
          model: 'llama-3.1-8b-instant',
  ...
  describe('generateIdeas()', () => {
    test('returns array of ideas', async () => {
      ...
      const result = await groqService.generateIdeas({
        genre: 'fantasy',
        count: 3,
      });
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        json: async () => ({
          choices: [
            {
              message: {
                content: 'The dark, stormy night engulfed the countryside...',
              },
            },
          ],
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        json: async () => ({
          choices: [
            {
              message: {
                content: 'A brave knight embarks on a quest to save the world.',
              },
            },
          ],
          model: 'llama-3.1-8b-instant',
