...
if (!RPC_URL || !PRIVATE_KEY || !CONTRACT_ADDRESS) {
  console.warn(
    '⚠️ Blockchain config missing. Worker will fail if blockchain ops are attempted.'
  );
}
...
function getContract() {
  if (!RPC_URL)
    throw new Error("Blockchain Config Error: Missing 'MONAD_RPC_URL'");
  if (!PRIVATE_KEY)
    throw new Error(
      "Blockchain Config Error: Missing 'MINT_AUTHORITY_PRIVATE_KEY'"
    );
  if (!CONTRACT_ADDRESS)
    throw new Error(
      "Blockchain Config Error: Missing 'NEXT_PUBLIC_CONTRACT_ADDR'"
    );

  if (!ethers.isAddress(CONTRACT_ADDRESS)) {
    throw new Error(
      `Blockchain Config Error: Invalid Contract Address format '${CONTRACT_ADDRESS}'`
    );
  }
