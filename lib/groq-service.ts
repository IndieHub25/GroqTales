...
/** Shape returned by story analysis (sentiment, themes, readability). */
...
/** A single story recommendation with similarity score and reason. */
...
export async function generateStoryWithProConfig(
  ...
  try {
    ...
    const systemPromptContent = buildProPanelSystemPrompt(
      proConfig,
      options.title
    );
    ...
    const enhancedUserPrompt = buildProPanelUserPrompt(
      sanitizedPrompt,
      proConfig
    );
 ...
 */
function buildProPanelSystemPrompt(
  config: ProParameters,
  title?: string
): string {
  const {
    storyStructure,
  ...
  const parts: string[] = [
    ...
    theme.secondaryThemes.length > 0
      ? `- Secondary themes: ${theme.secondaryThemes.join(', ')}`
      : '',
    '',
    '## Format:',
 ...
 */
function buildProPanelUserPrompt(
  basePrompt: string,
  config: ProParameters
): string {
  ...
  if (config.advanced.contentWarnings.length > 0) {
    contextParts.push(
      `- Be mindful of sensitive topics: ${config.advanced.contentWarnings.join(', ')}`
    );
  }
  ...
  if (config.advanced.seedPhrase) {
    contextParts.push(
      `- Maintain consistency with this seed: "${config.advanced.seedPhrase}"`
    );
  }
