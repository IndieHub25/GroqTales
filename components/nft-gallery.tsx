...
import { useRouter } from 'next/navigation'; // ✅ CHANGE 1: Added useRouter import
...
export function NFTGallery() {
  ...
  const fetchStories = async () => {
    try {
      const response = await fetch(`\${process.env.NEXT_PUBLIC_API_URL || 'https://groqtales-backend-api.onrender.com'}/api/stories`);
            ...
            <Card key={story.id} className="overflow-hidden">
              ...
              <CardFooter className="flex justify-between items-center gap-2">
                ...
                <Button
                  variant="default"
                  size="sm"
                  onClick={() => router.push(`/nft/${story.id}`)}
                >
                  Buy Now
                </Button>
              </CardFooter>
