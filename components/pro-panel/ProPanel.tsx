...
import { useProPanelStore, selectUI, selectStoryInput } from '@/store/proPanelStore';
import { CATEGORY_CONFIG, GENRE_PRESET_META } from '@/lib/constants/proPanelDefaults';
import { BookOpen, User, Cake, ClipboardList, Search, LayoutList, Columns } from 'lucide-react';
...
// Section components for accordion view
...
export function ProPanel() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<CategoryKey>('storyStructure');
  const [displayedCategory, setDisplayedCategory] = useState<CategoryKey>('storyStructure');
  ...
  return (
    <div className="font-display bg-[#050505] text-white relative min-h-screen">
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <main className="flex-grow container mx-auto px-4 py-8 max-w-7xl">
          ...
          <div className="flex justify-center items-center gap-4 mb-12">
            {STEP_TAGS.map((tag, index) => (
              <React.Fragment key={tag.step}>
                <div className={`flex flex-col items-center ${index === 0 ? 'rotate-3 translate-y-2' : index === 1 ? '-rotate-2' : 'rotate-1 translate-y-2'}`}>
                  <div className={`px-4 py-1 font-marker text-sm border-2 border-black shadow-comic ${tag.active ? 'bg-white text-black' : 'bg-gray-400 text-gray-700'
                    }`}>
                    TAG: {tag.label}
                  </div>
                  <div className={`w-1 h-8 ${tag.active ? 'bg-black' : 'bg-black/40'}`} />
                </div>
          ...
          <div className="text-center mb-12">
            <h1 className="font-marker text-[#8a0000] text-6xl md:text-8xl lg:text-9xl -rotate-2 drop-shadow-[8px_8px_0_rgba(0,0,0,1)] text-stroke-black leading-tight">
              {ui.selectedGenre ? ui.selectedGenre.toUpperCase() + ' DOSSIER' : 'NOIR DOSSIER'}
            </h1>
          ...
          {/* ── Main Content Grid ── */}
          <form className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start" onSubmit={(e) => e.preventDefault()}>
            ...
            <div className="lg:col-span-8 space-y-12">
              ...
              <section className="relative">
                ...
                <div className="bg-[#E5E5E5] border-[8px] border-black p-8 pt-12 shadow-comic-red relative overflow-hidden">
                  ...
                  <div className="absolute top-0 right-0 w-32 h-32 opacity-10 rotate-12 pointer-events-none"><Search className="w-full h-full text-black" /></div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                    <div className="space-y-2">
                      <label className="flex items-center gap-2 font-condensed text-black font-bold uppercase tracking-tighter">
                        <User className="w-4 h-4 text-[#8a0000]" /> Main Character Name
                      </label>
                      <input
                        type="text"
                        value={storyInput.characterName}
                        onChange={(e) => updateStoryInput('characterName', e.target.value)}
                    ...
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        onChange={(e) => updateStoryInput('ageRange', e.target.value)}
                  ...
                  <div className="bg-white/50 border-4 border-black p-6 relative">
                    <label className="font-condensed text-black font-bold uppercase mb-2 flex items-center gap-2">
                      <ClipboardList className="w-4 h-4 text-[#8a0000]" /> Character Background
                    </label>
                    <textarea
                      value={storyInput.characterBackground}
                      onChange={(e) => updateStoryInput('characterBackground', e.target.value)}
                ...
                <>
                        ...
                        return (
                          ...
                          >
                            {(() => {
                              ...
                              return IconComp ? <IconComp className={`w-3.5 h-3.5 inline-block mr-1 ${isActive ? 'text-white' : 'text-gray-400'}`} /> : null;
                            })()}
                            {config.title}
                          </button>
                        );
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-10 gap-0">
                    ...
                    <div className="lg:col-span-8 [perspective:1200px]">
                      <div ref={pageRef} style={{ transformOrigin: 'left center' }}>
                        <DossierFormPanel selectedCategory={displayedCategory} />
                      </div>
                    </div>
                  </div>
                </>
            ...
            <div className="lg:col-span-12 mt-12 mb-24 flex flex-col items-center gap-8">
              ...
              <div className="max-w-xl text-center">
                <p className="font-condensed text-gray-500 uppercase tracking-widest text-xs">
                  CAUTION: Narrative synthesis using Groq AI involves heavy gritty filters and unexpected plot twists. Proceed at your own peril, detective.
                </p>
              </div>
            </div>
          </form>
        ...
        {/* ── Footer ── */}
        <footer className="bg-black border-t-8 border-[#8a0000] p-12 relative overflow-hidden">
          <div className="absolute inset-0 halftone-bg opacity-10 pointer-events-none" />
          <div className="container mx-auto max-w-7xl grid grid-cols-1 md:grid-cols-4 gap-12 relative z-10">
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <BookOpen className="w-7 h-7 text-white" />
                <h2 className="text-white font-marker text-3xl tracking-widest">GROQTALES</h2>
              </div>
              <p className="font-condensed text-gray-400 text-sm uppercase tracking-wider">
                Empowering creators with AI-driven storytelling and Web3 ownership.
              </p>
              <div className="flex gap-4">
                <div className="w-8 h-8 border-2 border-[#8a0000] flex items-center justify-center text-[#8a0000] hover:bg-[#8a0000] hover:text-black cursor-pointer transition-colors font-condensed font-bold text-xs">FB</div>
                <div className="w-8 h-8 border-2 border-[#8a0000] flex items-center justify-center text-[#8a0000] hover:bg-[#8a0000] hover:text-black cursor-pointer transition-colors font-condensed font-bold text-xs">TW</div>
                <div className="w-8 h-8 border-2 border-[#8a0000] flex items-center justify-center text-[#8a0000] hover:bg-[#8a0000] hover:text-black cursor-pointer transition-colors font-condensed font-bold text-xs">IG</div>
              </div>
            </div>
            <div>
              <h4 className="font-marker text-[#8a0000] text-xl mb-6">THE PRECINCT</h4>
              <ul className="font-condensed space-y-2 text-gray-400 uppercase font-bold text-sm tracking-widest">
                <li><a className="hover:text-white transition-colors" href="#">Evidence Locker</a></li>
                <li><a className="hover:text-white transition-colors" href="#">Mugshots</a></li>
                <li><a className="hover:text-white transition-colors" href="#">Case Files</a></li>
                <li><a className="hover:text-white transition-colors" href="#">Informants</a></li>
              </ul>
            </div>
            <div>
              <h4 className="font-marker text-[#8a0000] text-xl mb-6">RESOURCES</h4>
              <ul className="font-condensed space-y-2 text-gray-400 uppercase font-bold text-sm tracking-widest">
                <li><a className="hover:text-white transition-colors" href="#">Documentation</a></li>
                <li><a className="hover:text-white transition-colors" href="#">FAQ</a></li>
                <li><a className="hover:text-white transition-colors" href="#">Contact Sergeant</a></li>
                <li><a className="hover:text-white transition-colors" href="#">Bribes &amp; Pricing</a></li>
              </ul>
            </div>
            <div className="border-4 border-[#8a0000] p-4 bg-gray-900 rotate-2">
              <h4 className="font-marker text-white text-xl mb-2">TOP SECRET INTEL</h4>
              <p className="font-condensed text-[10px] text-gray-400 uppercase mb-4">
                Powered by hyper-speed neural networks, this device synthesizes narrative structures faster than a speeding bullet!
              </p>
              <div className="flex justify-between items-center border-t border-[#8a0000] pt-2">
                <span className="font-condensed font-bold text-[#8a0000] text-[10px]">VERIFIED BY MONAD</span>
                <span className="text-white text-sm">✓</span>
              </div>
            </div>
          </div>
          <div className="container mx-auto max-w-7xl mt-12 pt-8 border-t border-gray-800 flex flex-col md:flex-row justify-between items-center gap-4 relative z-10">
            <p className="font-condensed text-gray-600 text-xs uppercase tracking-[0.3em]">© 2026 GROQTALES • BUILT BY INDIE HUB</p>
            <div className="flex gap-6 font-condensed text-gray-600 text-[10px] uppercase">
