...
import { useProPanelStore, selectParameters, selectUI, selectStoryInput } from '@/store/proPanelStore';
import {
    Castle, Rocket, Ghost, Search, Heart, Map, Theater, Building2, Crosshair, Drama,
    Clock, Cloud, Pencil, FileText, Meh
} from 'lucide-react';
...
export function LocationDocksPanel() {
    ...
    const SETTING_TYPES = [
        ...
        'low-fantasy', 'high-fantasy', 'science-fiction', 'post-apocalyptic',
        'dystopian', 'urban-fantasy', 'steampunk', 'cyberpunk',
    ] as const;

    const TECHNOLOGY_LEVELS = [
        'primitive', 'medieval', 'renaissance', 'industrial',
        'modern', 'near-future', 'far-future', 'mixed',
    ] as const;
            ...
            <div className="p-4 space-y-4">
                <div className="space-y-2">
                    ...
                    <select
                        ...
                        onChange={(e) => updateParameter('world', 'technologyLevel', e.target.value as typeof TECHNOLOGY_LEVELS[number])}
                ...
                <div className="space-y-2">
                    ...
                    <select
                        ...
                        onChange={(e) => updateParameter('world', 'settingType', e.target.value as typeof SETTING_TYPES[number])}
                        className="w-full bg-white border-2 border-black p-2 font-condensed text-sm text-black uppercase font-bold focus:ring-0 focus:outline-none"
                    >
                        {SETTING_TYPES.map(s => (
...
export function StyleDossierPanel() {
    ...
    const PROSE_STYLES = ['minimalist', 'literary', 'pulpy', 'journalistic', 'lyrical', 'cinematic', 'conversational'] as const;
    const TONE_OPTIONS = ['serious', 'humorous', 'satirical', 'dark', 'hopeful', 'melancholic', 'suspenseful', 'romantic', 'whimsical', 'gritty'] as const;
            ...
            <div className="space-y-4">
                <div className="space-y-1">
                    ...
                    <select
                        ...
                        onChange={(e) => updateParameter('toneStyle', 'proseStyle', e.target.value as typeof PROSE_STYLES[number])}
                        className="w-full bg-white border-2 border-black p-2 font-condensed text-sm text-black uppercase font-bold focus:ring-0 focus:outline-none"
                    >
                        {PROSE_STYLES.map(s => (
                            <option key={s} value={s}>{formatLabel(s)}</option>
                ...
                <div className="space-y-1">
                    ...
                    <select
                        ...
                        onChange={(e) => updateParameter('toneStyle', 'primaryTone', e.target.value as typeof TONE_OPTIONS[number])}
                        className="w-full bg-white border-2 border-black p-2 font-condensed text-sm text-black uppercase font-bold focus:ring-0 focus:outline-none"
                    >
                        {TONE_OPTIONS.map(t => (
                            <option key={t} value={t}>{formatLabel(t)}</option>
                ...
                <div className="space-y-2 pt-4 border-t border-black/20">
                    ...
                    <input
                        ...
                        onChange={(e) => updateParameter('toneStyle', 'humorLevel', Number(e.target.value))}
                ...
                <div className="space-y-2 pt-4 border-t border-black/20">
                    <div className="bg-black text-white p-3 border-2 border-[#8a0000] rotate-1">
                        <h4 className="font-condensed font-bold text-xs uppercase mb-2">CONTENT CONTROLS</h4>
                        <div className="flex items-center justify-between text-[10px] uppercase font-bold font-condensed">
                            <span>Violence</span>
                            <span className="text-[#8a0000]">{formatLabel(tone.violenceLevel)}</span>
                        </div>
                        <div className="flex items-center justify-between text-[10px] uppercase font-bold font-condensed mt-1">
                            <span>Language</span>
                            <span className="text-[#8a0000]">{formatLabel(tone.romanceLevel)}</span>
                        </div>
                    </div>
                </div>
            </div>
...
export function ContentControlsPanel() {
    ...
    const VIOLENCE_OPTIONS = ['none', 'mild', 'moderate', 'graphic', 'extreme'] as const;
    const ROMANCE_OPTIONS = ['none', 'subtle', 'moderate', 'prominent', 'central'] as const;
    ...
    return (
        <div className="bg-[#E5E5E5] border-[6px] border-black p-6 shadow-comic">
            <div className="bg-black text-white p-4 border-2 border-[#8a0000] rotate-1">
                <h4 className="font-condensed font-bold text-sm uppercase mb-3 tracking-wider">CONTENT CONTROLS</h4>
                <div className="space-y-3">
                    ...
                    <select
                        ...
                        onChange={(e) => updateParameter('toneStyle', 'violenceLevel', e.target.value as typeof VIOLENCE_OPTIONS[number])}
                    ...
                    <select
                        ...
                        onChange={(e) => updateParameter('toneStyle', 'romanceLevel', e.target.value as typeof ROMANCE_OPTIONS[number])}
                        className="w-full bg-gray-900 border border-gray-600 p-2 font-condensed text-xs text-white uppercase font-bold focus:ring-0 focus:outline-none"
                    >
...
// ============================================================
const TROPES_AVOID = ['Chosen One', 'Love Triangle', 'Deus Ex Machina', 'Amnesia Plot', 'Evil Twin'];
const TROPES_INCLUDE = ['Hero Journey', 'Mentor Figure', 'Found Family', 'Redemption Arc', 'Underdog Story'];

export function InspirationPanel() {
    const storyInput = useProPanelStore(selectStoryInput);
    const updateStoryInput = useProPanelStore((s) => s.updateStoryInput);

    const toggleTrope = (list: 'tropesToAvoid' | 'tropesToInclude', trope: string) => {
    ...
    return (
        <div className="bg-[#8a0000] p-5 border-4 border-black shadow-comic space-y-4 -rotate-1">
            {/* Title */}
            <h3 className="font-marker text-white text-xl tracking-wider uppercase">Inspiration</h3>
            ...
            <div className="space-y-1">
                ...
                <input
                    ...
                    onChange={(e) => updateStoryInput('inspirationPrompt', e.target.value)}
            ...
            <div className="space-y-1">
                ...
                <input
                    ...
                    onChange={(e) => updateStoryInput('inspirationAuthor', e.target.value)}
                    className="w-full bg-gray-900 border-2 border-gray-700 p-2 font-condensed text-xs text-white placeholder:text-gray-500 focus:ring-0 focus:outline-none focus:border-white"
                />
            </div>
