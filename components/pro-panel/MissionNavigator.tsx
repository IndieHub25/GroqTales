                            ...
                            >
                                ...
                                <span className="shrink-0 w-7 flex items-center justify-center">
                                    {(() => {
                                        ...
                                        return IconComp
                                            ? <IconComp className={`w-5 h-5 ${isActive ? 'text-white' : 'text-gray-400'}`} />
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                        <span className={`font-condensed text-[10px] uppercase tracking-wider ${isActive ? 'text-white/70' : 'text-gray-600'}`}>
                                            FILE {fileNum}
                                        </span>
                                        {isActive && (
                                    ...
                                    <p className={`font-condensed font-bold uppercase text-xs tracking-wider truncate ${isActive ? 'text-white' : ''}`}>
                                        {config.title}
                                    </p>
                                </div>

                                {/* Arrow indicator */}
                                <span className={`text-xs transition-transform shrink-0 ${isActive ? 'text-white' : 'text-gray-600 group-hover:text-gray-400'}`}>
                                    {isActive ? '▶' : '›'}
                                </span>
                            </button>
                        );
