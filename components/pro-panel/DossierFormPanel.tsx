...
import {
    BookOpen, Users, Globe, Theater, Gem, Bot, Palette, Ruler, Settings, Lightbulb
} from 'lucide-react';
import { ICON_MAP } from './iconMap';

// Section components — one per category
...
const DOSSIER_META: Record<CategoryKey, {
    fileNumber: string;
    docTitle: string;
    stamps: string[];
    watermark: React.ReactNode;
}> = {
    storyStructure: { fileNumber: '01', docTitle: 'NARRATIVE OPERATIONS BRIEFING', stamps: ['PLOT APPROVED', 'STRUCTURE VERIFIED'], watermark: <BookOpen className="w-full h-full text-black" /> },
    characters: { fileNumber: '02', docTitle: 'PERSONNEL DOSSIER', stamps: ['IDENTITY CONFIRMED', 'BACKGROUND CLEARED'], watermark: <Users className="w-full h-full text-black" /> },
    world: { fileNumber: '03', docTitle: 'LOCATION INTELLIGENCE REPORT', stamps: ['LOCATION SECURED', 'TERRAIN MAPPED'], watermark: <Globe className="w-full h-full text-black" /> },
    toneStyle: { fileNumber: '04', docTitle: 'COMMUNICATION PROTOCOL', stamps: ['TONE AUTHORIZED', 'STYLE APPROVED'], watermark: <Theater className="w-full h-full text-black" /> },
    theme: { fileNumber: '05', docTitle: 'OPERATIONAL THEMES & OBJECTIVES', stamps: ['THEMES CLASSIFIED', 'OBJECTIVES SET'], watermark: <Gem className="w-full h-full text-black" /> },
    modelSettings: { fileNumber: '06', docTitle: 'TECHNICAL SPECIFICATIONS - CLASSIFIED', stamps: ['TECH AUTHORIZED', 'AI CLEARANCE LVL 5'], watermark: <Bot className="w-full h-full text-black" /> },
    visual: { fileNumber: '07', docTitle: 'VISUAL INTELLIGENCE BRIEFING', stamps: ['VISUAL APPROVED', 'COVER ART CLEARED'], watermark: <Palette className="w-full h-full text-black" /> },
    length: { fileNumber: '08', docTitle: 'DOCUMENT SPECIFICATIONS', stamps: ['LENGTH CONFIRMED', 'FORMAT APPROVED'], watermark: <Ruler className="w-full h-full text-black" /> },
    advanced: { fileNumber: '09', docTitle: 'SPECIAL OPERATIONS PARAMETERS', stamps: ['CLASSIFIED', 'AUTHORIZED ONLY'], watermark: <Settings className="w-full h-full text-black" /> },
};
                ...
                <div>
                    ...
                    <div className="flex items-center gap-3 mt-2">
                        <span className="font-condensed text-[10px] text-gray-500 uppercase tracking-wider">
                            FILE NO. {meta.fileNumber}-{selectedCategory.slice(0, 4).toUpperCase()}
                        </span>
            ...
            <div className="relative z-10 border-b-2 border-dashed border-black/20 px-8 lg:px-12 py-3 bg-black/[0.03]">
                <div className="flex items-center gap-3">
                    {(() => {
                        ...
                        return IconComp ? <IconComp className="w-6 h-6 text-black" /> : null;
                    })()}
                    <div>
