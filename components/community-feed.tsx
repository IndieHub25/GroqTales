...
const showcaseNFTs = [
  ...
  {
    id: 2,
    img: 'https://images.unsplash.com/photo-1446776877081-d282a0f896e2?w=400&h=400&fit=crop',
    title: 'Quantum Drop',
    author: 'Sarah',
  },
  {
  ...
  return (
    <div className="flex items-center justify-between text-white/50 pt-4 mt-2 border-t border-white/10">
      <div className="flex items-center gap-1 bg-white/5 rounded-full p-1 border border-white/10">
        ...
        <span
          className={`text-sm font-semibold min-w-[2ch] text-center ${post.userVote === 'up' ? 'text-emerald-400' : post.userVote === 'down' ? 'text-rose-400' : 'text-white/70'}`}
        >
          {post.likes}
        </span>
        <button
      ...
      <div className="bg-black/60 border border-white/10 rounded-3xl p-6 backdrop-blur-md transition-colors hover:border-white/20">
          ...
          <div className="flex gap-3 items-center">
            ...
            <div>
              ...
              <div className="text-xs text-white/40">
                {Math.floor((Date.now() - post.timestamp.getTime()) / 3600000)}h
                ago
              </div>
            </div>
          </div>
          <Badge className="bg-white/5 text-white/50 border-white/10 uppercase tracking-wider text-[10px]">
            {post.type}
          </Badge>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {post.genre?.map((g) => (
            <Badge
              key={g}
              className="bg-emerald-500/10 text-emerald-400 border-none hover:bg-emerald-500/20"
            >
              {g}
            </Badge>
          ))}
...
export default function CommunityFeed() {
  ...
  useEffect(() => {
    ...
    fetch(`\${process.env.NEXT_PUBLIC_API_URL || ''}/api/feed?limit=20`)
      .then((r) => {
      ...
      .then((json) => {
        ...
        const mapped = data.map((story: any, i: number) => ({
          ...
          author: {
            ...
            avatar:
              story.coverImage ||
              `https://api.dicebear.com/7.x/personas/svg?seed=${story.authorName || i}`,
            level: 1,
            verified: false,
          },
          content:
            story.summary ||
            story.content ||
            `Check out this latest community addition: ${story.title || 'Untitled'}`,
          title: story.title,
          genre: Array.isArray(story.genre)
            ? story.genre
            : story.genre
              ? [story.genre]
          ...
          comments:
            story.commentsCount || story.comments_count || story.comments || 0,
          shares: 0,
  ...
  const filtered = posts.filter(
    (p) =>
      filter === 'all' ||
      p.type === (filter.endsWith('s') ? filter.slice(0, -1) : filter)
  );
        ...
        <div className="grid lg:grid-cols-12 gap-8">
              ...
              {loading ? (
                ...
                <div className="text-center py-12 bg-white/5 rounded-3xl border border-white/10">
                  ...
                  <p className="text-white/50">
                    The community feed is currently quiet.
                  </p>
                </div>
              ) : (
                <AnimatePresence>
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <motion.div
              ...
              className="bg-white/5 border border-white/10 rounded-3xl p-6 backdrop-blur-md relative overflow-hidden group"
            >
                ...
                <div className="flex items-center gap-4 mb-6">
                  <div className="relative">
                    <Avatar className="h-16 w-16 border-2 border-emerald-500">
                      <AvatarImage
                        src={`https://api.dicebear.com/7.x/personas/svg?seed=${profile?.username || 'You'}`}
                      />
                      <AvatarFallback>
                        {profile?.username
                  ...
                  <div>
                    <h3 className="font-bold text-lg text-white">
                      {profile?.username || 'Guest Creator'}
                    </h3>
                    <p className="text-sm text-emerald-400">
                      {profile?.rank || 'Novice Scribe'}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-xs font-semibold text-white/50">
                    ...
                    <span>
                      ...
                      {(profile?.nextLevelXp || 2000).toLocaleString()}
                    </span>
                  </div>
                  <div className="w-full bg-black/50 rounded-full h-3 border border-white/10 overflow-hidden relative">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{
                        width: `${Math.min(100, ((profile?.xp || 0) / (profile?.nextLevelXp || 2000)) * 100)}%`,
                      }}
                  ...
                  <p className="text-center text-[10px] text-white/40 pt-1">
                    {(profile?.nextLevelXp || 2000) - (profile?.xp || 0)} XP to
              ...
              <div className="space-y-4">
                {showcaseNFTs.map((nft) => (
                  ...
                  >
                    ...
                    <div>
                      <h4 className="font-bold group-hover:text-emerald-400 transition-colors line-clamp-1">
                        {nft.title}
                      </h4>
