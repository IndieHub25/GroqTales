    ...
    if (req.query.action === 'test') {
      ...
      if (req.query.apiKey) {
        return res.status(400).json({
          error:
            'API key must be provided via Authorization header (Bearer token). Query parameter authentication is not supported for security reasons.',
        });
      }
  ...
  try {
    ...
    switch (action) {
      case 'generate': {
        ...
        if (!prompt && !theme) {
          return res
            .status(400)
            .json({
              error: 'Either prompt or theme is required for generation',
            });
        }
      ...
      case 'analyze': {
        ...
        result = await groqService.analyze({
      ...
      case 'ideas': {
        ...
        const safeCount = Number.isFinite(rawCount)
          ? Math.min(Math.max(rawCount, 1), 20)
          : 5;
        result = await groqService.generateIdeas({
          genre: normalizedGenre,
          theme: normalizedTheme,
          count: safeCount,
          apiKey,
      ...
      case 'improve': {
        ...
        if (!content) {
          return res
            .status(400)
            .json({ error: 'content is required for improvement' });
        }
        ...
        result = await groqService.improve({
          content: normalizedContent,
      ...
      default:
        return res
          .status(400)
          .json({
            error: `Unknown action: ${action}. Supported: generate, analyze, ideas, improve`,
          });
    }
  } catch (error) {
