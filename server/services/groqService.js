...
function buildStoryPrompt({
  ...
  const parts = [
    `Write a ${length || 'medium'}-length ${formatType || 'story'}`,
  ];
  ...
  parts.push(
    '. Include vivid descriptions, dialogue, and a satisfying arc. Start immediately with the narrative — no preamble.'
  );
  return parts.join(' ');
}
...
function buildSynopsisPrompt(content, genre, formatType) {
  ...
  return (
    `Write a compelling 2-sentence synopsis for this ${formatType || 'story'} (${genre || 'fiction'}). ` +
...
function buildImprovementPrompt(content, focusArea) {
  ...
  return (
    `Improve this text${focusArea ? `, focusing on ${focusArea}` : ''}. ` +
...
const SYSTEM_PROMPTS = {
  story:
    'You are a master storyteller. Write engaging, vivid narratives. Never break character or add meta-commentary.',
  comic:
    'You are a comic script writer. Output panel-by-panel descriptions with dialogue. Visual, concise, cinematic.',
  novel:
    'You are a novelist. Write immersive prose with strong characters and pacing. Chapter format.',
  analysis:
    'You are a literary analyst. Return ONLY valid JSON — no markdown fences, no explanation.',
  synopsis: 'You write compelling blurbs. Concise, intriguing, spoiler-free.',
  ideas:
    'You generate creative story premises. Each is unique, specific, and compelling.',
  improve:
    "You are an expert editor. Enhance the text while preserving the author's voice. Return only the improved text.",
    ...
    try {
        ...
        'https://api.groq.com/openai/v1/chat/completions',
        {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: 'system',
                content: systemPrompt || SYSTEM_PROMPTS.general,
              },
      ...
      if (!response.ok) {
        ...
        if (response.status >= 500 && attempt < MAX_RETRIES) {
          ...
          logger.warn(
            `Groq API ${response.status}, retrying in ${delay}ms (attempt ${attempt + 1}/${MAX_RETRIES})`
          );
...
async function generate({
  ...
  if (ft === 'comic' || ft === 'comics') {
    userPrompt =
      prompt || buildComicPrompt({ genre, theme, characters, setting });
    systemPrompt = SYSTEM_PROMPTS.comic;
    budget = TOKEN_BUDGETS.long;
  } else if (ft === 'novel' || ft === 'book') {
    userPrompt =
      prompt || buildNovelPrompt({ genre, theme, characters, setting, tone });
