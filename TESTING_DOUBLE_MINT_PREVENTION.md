...
This guide walks through testing the idempotency protections added to prevent accidental
double-minting of stories.

## What Was Implemented

1. **Content Hash Tracking** – Each story gets a unique hash before minting (already in
   `components/story-generator.tsx`)
2. **Minting Status Check** – `/api/mint/check` endpoint validates minting status before allowing
   new mints
